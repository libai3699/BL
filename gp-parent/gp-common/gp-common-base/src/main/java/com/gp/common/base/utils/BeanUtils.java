...
public class BeanUtils {
    ...
    public static <T1,T2> List<T2> copyListPropertiesIgnoreNull(List<T1> src, Class<T2> clazz) {
        ...
        for (T1 src1 : src) {
            ...
            BeanUtils.copyPropertiesIgnoreNull(src1,t2);
            targers.add(t2);
        }
        return targers;
    }
    @SneakyThrows
    public static <T1,T2> List<T2> copyListProperties(List<T1> src, Class<T2> clazz) {
        ArrayList<T2> targers = new ArrayList<>();
        for (T1 src1 : src) {
            T2 t2 = clazz.newInstance();
            BeanUtils.copyPropertiesIgnoreNull(src1, t2);
    ...
    @SneakyThrows
    public static <T2>  T2  copyPropertiesIgnoreNull(Object src, Class<T2> clazz) {
        T2 t2 = clazz.newInstance();
        BeanUtils.copyPropertiesIgnoreNull(src,t2);
        return t2;
    }
    /**
     * 去掉字符串中斜线后, 再转换为Map
     ...
     * @throws IllegalAccessException
     */
    public static TreeMap<String, String> objToMapForSign(Object object,Boolean isNull) throws IllegalAccessException {
        if (isNull) {
            return objToMapForSign(object);
        }else {
            Class clazz = object.getClass();
            ...
            while (null != clazz.getSuperclass()) {
                ...
                for (Field field : declaredFields1) {
                    ...
                    if (null != value && !field.getName().equals("sign")) {
                        //如果是List,将List转换为json字符串
                        if (value instanceof List) {
                            value = JSON.toJSONString(value);
                        }
