...
public class BigDecimalUtils {
     ...
     * 去除无用的0
     *
     * @param num 被加数（可为null）
     * @author dengcs
     */
    public static String trim(BigDecimal num) {
        if (num == null) {
            return "";
        }
    ...
    public static BigDecimal divide(BigDecimal x, BigDecimal y) {
        ...
        return stripTrailingZeros(x.divide(y, 8, BigDecimal.ROUND_DOWN));
    }
    ...
    public   static String dealPercent(BigDecimal value) {
        // 将BigDecimal值转换为百分比
        // 这是因为百分比就是按照每一百为单位的比例，所以0.25(即1/4)变成百分比就是25%
        BigDecimal percentageValue = value.multiply(new BigDecimal("100"));

        // 使用DecimalFormat来格式化百分比，包括设置小数点后位数和添加百分号
        BigDecimal result = percentageValue.setScale(2, RoundingMode.DOWN);
        return BigDecimalUtils.trim(result) + "%";
    }

    public static BigDecimal digit10() {
       return  new BigDecimal("10000000000");
    }
    public static BigDecimal digitPix5W() {
        return  new BigDecimal("50000");
    }

    public static <T> BigDecimal sumBigDecimalField(List<T> orders, Function<T, BigDecimal> getter) {
        return orders.stream()
                .filter(it -> Objects.nonNull(getter.apply(it)))
                .map(getter)
    ...
    public static BigDecimal percentageToDecimal(BigDecimal percentage) {
        return percentage.divide(new BigDecimal("100"), 4, RoundingMode.DOWN);
    }
     ...
     * @return 百分比值
     */
    public static BigDecimal decimalToPercentage(BigDecimal decimal, int scale) {
