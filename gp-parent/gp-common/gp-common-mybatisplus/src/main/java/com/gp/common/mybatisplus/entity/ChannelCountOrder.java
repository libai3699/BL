...
/**
 * 渠道每日订单统计对象 t_channel_count_order
 *
 * @author axing
 * @date 2024-05-24
 */
...
@NoArgsConstructor
@AllArgsConstructor
@TableName("t_channel_count_order")
public class ChannelCountOrder extends BaseEntity {
    private static final long serialVersionUID = 1L;
    public static final String REDIS_KEY = "msg:ChannelCountOrder";
     ...
     * 用户剩余积分
     */
    @ApiModelProperty("渠道用户剩余积分")
    @TableField("usdt_balance")
    @Excel(name = "渠道用户剩余积分", cellType = Excel.ColumnType.NUMERIC)
    ...
    /**
     * 赠送总彩金（赠送合计减去彩金扣款 customerPointsDeductedAmount）
     */
    @TableField(exist = false)
    ...
    @Excel(name = "已返水金额", cellType = Excel.ColumnType.NUMERIC)
     ...
     * 待返佣金额-下级用户返水返给自己的金额
     */
    @ApiModelProperty("待返佣金额-下级用户返水返给自己的金额")
    ...
    @Excel(name = "客损([计算公式 等于 输赢 加 所有送的金额 减 手动下分(扣除积分)])", cellType = Excel.ColumnType.NUMERIC)
     ...
     */
    @TableField(exist = false)
    @ApiModelProperty(value = "当月活跃人数")
//    @Excel(name = "当月活跃人数", cellType = Excel.ColumnType.NUMERIC)
    private Integer totalActivePeopleNum;
    ...
    @Excel(name = "超级红包数量", cellType = Excel.ColumnType.NUMERIC)
    ...
    @Excel(name = "超级群红包金额", cellType = Excel.ColumnType.NUMERIC)
