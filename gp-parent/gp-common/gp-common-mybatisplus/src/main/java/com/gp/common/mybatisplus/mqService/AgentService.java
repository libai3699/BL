...
public class AgentService {
    ...
    public void computeBrokerageUserCountOrder(
        ...
        if (userCountOrder == null) {
            ...
            userCountOrder.setUserAmount(amount.getUserAmount());//用户结余-当日(加上工资)
            userCountOrder.setUserEarnings(orderEntity.getMoney().add(orderEntity.getMoney()));//用户收入+领取的工资
            ...
            userCountOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
            int result = userCountOrderMapper.saveUserCountOrderData(userCountOrder);
            log.info("uuid={}, 领取代理工资订单->添加用户数据: {} 结果: {}", orderEntity.getUuid(), userCountOrder.toString(), result);
        } else {
            UserCountOrder updateUserCountOrder = new UserCountOrder();
            updateUserCountOrder.setId(userCountOrder.getId());
            updateUserCountOrder.setUserAmount(amount.getUserAmount());//用户结余-当日(加上工资)
            updateUserCountOrder.setUserEarnings(orderEntity.getMoney().add(orderEntity.getBonusAmount()));//用户收入+领取的工资
            updateUserCountOrder.setCustomerLossAmount(orderEntity.getMoney());
            updateUserCountOrder.setCalculationsTime(new Date());//操作时间
            updateUserCountOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
            updateUserCountOrder.setUpdateTime(new Date());
            int result = userCountOrderMapper.updateUserCountOrderData(updateUserCountOrder);
            log.info("uuid={}, 领取代理工资订单->更新用户数据: {} 更新结果: {}", orderEntity.getUuid(), updateUserCountOrder.toString(), result);
        }
    }
    ...
    public void computeBrokerageChannelCountOrder(
        ...
        if (channelCountOrder == null) {
            ...
            channelCountOrder.setUserAmount(amount.getChannelAmount());//用户结余-当日(加上工资)
            channelCountOrder.setUserEarnings(orderEntity.getMoney());//用户收入+领取的工资
            channelCountOrder.setCustomerLossAmount(orderEntity.getMoney());
            channelCountOrder.setCalculationsTime(new Date());//操作时间
            channelCountOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
//            channelCountOrder.setAlreadyReturnCommissionAmount(orderEntity.getMoney());
            ...
            log.info("uuid={}, 领取代理工资订单->添加渠道数据: {} 结果: {}", orderEntity.getUuid(), channelCountOrder.toString(), result);
        } else {
            ...
            updateChannelCountOrder.setUserAmount(amount.getChannelAmount());//用户结余-当日代理工资
            updateChannelCountOrder.setUserEarnings(orderEntity.getMoney());//用户收入+代理工资
...
//            updateChannelCountOrder.setAlreadyReturnCommissionAmount(orderEntity.getMoney());
            updateChannelCountOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
            ...
            log.info("uuid={}, 领取代理工资订单->更新渠道数据: {} 更新结果: {}", orderEntity.getUuid(), updateChannelCountOrder.toString(), result);
        }
    }
...
//            countOrder.setAlreadyReturnCommissionAmount(orderEntity.getMoney());
            countOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
            ...
            log.info("uuid={}, 领取代理工资订单->添加总览数据: {} 结果: {}", orderEntity.getUuid(), countOrder.toString(), result);
        } else {
...
//            updateCountOrder.setAlreadyReturnCommissionAmount(orderEntity.getMoney());
            updateCountOrder.setClaimedAgentSalaryAmount(orderEntity.getMoney());
            ...
            log.info("uuid={}, 领取代理工资订单->更新总览数据: {} 更新结果: {}", orderEntity.getUuid(), updateCountOrder.toString(), result);
        }
    }
}
